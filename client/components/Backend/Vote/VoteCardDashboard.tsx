import VoteStaticProgress from "../../Vote/VoteStaticProgress";
import ListVote from "../../Vote/ListVote";
import Link from "next/link";
import { Button } from "../../ui/button";

const VoteCardDashBoard = ({ vote, onRemove }) => {
  ...
  const voteAgreePercentage = parseFloat(((voteAgreeCount / voteAttendees) * 100).toFixed(2));
  const voteDisagreePercentage = parseFloat(((voteDisagreeCount / voteAttendees) * 100).toFixed(2));
  const voteAbstentionPercentage = parseFloat(((voteAbstentionCount / voteAttendees) * 100).toFixed(2));
  const voteAbstainPercentage = parseFloat(((voteAbstainCount / voteAttendees) * 100).toFixed(2));
  ...
  return (
    <div className="bg-custom-white p-4 rounded-lg border border-custom-light-2 my-2 transition-all cursor-pointer">
      ...
      <div className="flex">
          <Button variant="destructive" onClick={() => onRemove(Number(vote.id))}>ลบ</Button>
      </div>
    </div>
  );
}

export default VoteCardDashBoard;
