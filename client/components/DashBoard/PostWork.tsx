...
import { PostWorkProps } from '@/app/interfaces/Props/Work';
...
const PostWork: React.FC<PostWorkProps> = ({ listUser, workTag }) => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      formData.append("workPostBy", session?.user.id);
      if (data.workTagId) {
      ...
      data.workImages.forEach((imageForm, index) => {
        if (imageForm.image[0]) {
          formData.append(`workImage`, imageForm.image[0]);
        }
      });
      await AddWork(formData);
      toast.success(`โพสต์ "${data.workTitle}" เรียบร้อย`);
      reset();
      router.refresh();
    } catch (error) {
                    ...
                    >
                      <SelectTrigger className="w-full">
                        <SelectValue placeholder="เลือกผู้ดำเนินการ">
                          {listUser.find(user => user.idQuery == field.value)?.displayName || "เลือกผู้ดำเนินการ"}
                        </SelectValue>
                      </SelectTrigger>
                      <SelectContent>
                        {listUser.map((user) => (
                          <SelectItem key={user.idQuery} value={user.idQuery}>
                            {user.displayName}
                          </SelectItem>
                        ))}
                      </SelectContent>
              ...
              <Select onValueChange={field.onChange} value={field.value}>
                <SelectTrigger className="w-full">
                  <SelectValue placeholder="เลือกแท็ก">
                    {workTag.find(tag => tag.idQuery == field.value)?.tagTitle || "เลือกแท็ก"}
                  </SelectValue>
                </SelectTrigger>
                <SelectContent>
                  {workTag.map((tag, i) => (
                    <SelectItem key={tag.idQuery} value={tag.idQuery}>
                      {i + 1}. {tag.tagTitle}
                    </SelectItem>
                  ))}
                </SelectContent>
