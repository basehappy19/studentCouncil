import { Budget } from "@/app/functions/Budget";
import NavbarPink from "@/app/layouts/NavbarPink";
import { BudgetData } from "@/app/interfaces/Budget/Budget";
import { TransactionData } from "@/app/interfaces/Transaction/Transaction";
import { TransactionById } from "@/app/functions/Transaction";
...
export async function generateMetadata({
  ...
  const { id } = params;
  const budget: BudgetData[] = await getBudget(id);

  return {
    title: `งบประมาณ${budget[0].budgetTitle} ${process.env.NEXT_PUBLIC_APP_TITLE}`,
    description:
...
async function page({ params }: { params: { id: string } }) {
  const { id } = params;
  const budget: BudgetData[] = await getBudget(id);
  const transaction: TransactionData[] = await getTransaction(id);
  return (
      ...
      <section className="p-10">
          ...
          <div className="flex flex-col lg:flex-row flex-wrap justify-center">
            <div className={`w-full lg:w-1/2 mx-auto text-center`}>
              <div
                ...
                style={{ backgroundColor: budget[0].budgetColor }}>
                <div className="text-5xl text-custom-white font-semibold">
                  {budget[0].budgetTitle}
                </div>
              </div>
            </div>
            <div className="w-full lg:w-1/2 mx-auto text-center items-start">
              <div className="mt-12">
                ...
                <div className="text-2xl mt-4">
                  หน้าที่ของฝ่าย{budget[0].budgetTitle}
                </div>
                <div className="text-start text-custom-gray">
                  {budget[0].budgetDescription}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="container mx-auto py-8 px-4">
          <h1 className="text-center font-semibold mb-8 text-3xl">
            บัญชีงบประมาณฝ่าย{budget[0].budgetTitle}
          </h1>
...
async function getBudget(id: string): Promise<BudgetData[]> {
  const response = await Budget(id);
  return response;
}

async function getTransaction(id: string): Promise<TransactionData[]> {
  const response = await TransactionById(id);
  return response;
}
