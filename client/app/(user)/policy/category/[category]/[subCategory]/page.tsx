import { PolicyFilter } from "@/app/functions/Policy";
import {
  PolicyCategories,
  AllPolicyCategory,
} from "@/app/functions/Category";
...
export async function generateMetadata({
  ...
  const categoryData: CategoryData = await getPolicyCategories(category);
  const categorySrc =
    process.env.NEXT_PUBLIC_APP_POLICIES_CATEGORIES_IMG_PATH_SERVER || "";
  const policies: Policies[] = await getPolicyFilter(
    category,
    subCategory
  );
...
async function page({
  ...
  const subCategoryId: number = parseInt(subCategory);
    ...
    <div className="bg-custom-background pb-5">
                ...
                {categoryData.subCategories.map((subCategory) =>
                  subCategory.id === subCategoryId ? (
                    <div className="drop-shadow-lg">
                      <p
                        className="text-custom-secondary text-2xl text-center"
                        key={subCategory.id}
                      >
                        {subCategory.title}
                      </p>
      ...
      <section className="pt-10 container mx-auto px-4">
        <div className="w-auto mx-[-1.5rem] overflow-x-auto overflow-y-hidden px-[1.5rem] mb-3">
          <div className="flex gap-4">
            <div>
              <Link href={`/policy/category/${categoryData.id}`}>
                <div
                  className={`transition-all rounded-lg duration-300 py-2 px-8 whitespace-nowrap font-medium text-custom-black hover:bg-custom-primary hover:text-custom-white ${
...
async function getPolicyFilter(
  category: string,
  subCategory: string
) {
  const response = await PolicyFilter(category, subCategory);
  return response;
}
